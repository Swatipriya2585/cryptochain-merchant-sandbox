...
Log in (no API key required). The response includes merchant id, password, wallet addresses, and public/private keys for demo payments. Send the returned `apiKey` as a Bearer token:
...
| ETH wallet | `0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266` (Hardhat #0, demo only) |
| ETH private key | `0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80` |

A new email + password creates a merchant with unique Ethereum, Solana, and Bitcoin demo keypairs. **These keys are sandbox-only — never use them on mainnet.**
...
Public merchant testing page (no API key). Served with `res.sendFile`. Product nav includes **Sandbox**. The console requires login, then shows merchant id, password, wallet address, and public/private keys, plus Create Payment, History, Status, Webhook, and Profile:

- `POST /sandbox/merchant/login`
- `GET /sandbox/merchant/credentials`
- `GET /sandbox/merchant/profile`
- `PATCH /sandbox/merchant/settings`
- `POST /sandbox/pay`
...
No auth header required. Existing merchants must use the correct password. A new email creates a demo merchant with unique wallets. The pre-seeded tester account is `merchant@sandbox.test` / `sandbox123`.
...
```json
{
  ...
  "data": {
    ...
    "wallets": {
      "ethereum": { "address": "0xf39F…", "publicKey": "0x04…", "privateKey": "0xac09…" },
      "solana": { "address": "AZe3…", "publicKey": "AZe3…", "privateKey": "3a4m…" },
      "bitcoin": { "address": "mwDd…", "publicKey": "ac3b…", "privateKey": "cCbK…" }
    },
    "warning": "Sandbox demo keys only. Never use these on mainnet or with real funds."
  },
...
Returns the same demo credential vault as login: merchant id, password, API key, wallet addresses, and public/private keys.
...
```json
{ "webhookUrl": "https://merchant.example/webhooks/cryptochain", "businessName": "Sandbox Test Store" }
```
